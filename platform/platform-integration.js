 ...
 * Connects existing GEI module pages to the shared Platform Core.
 * Browser state remains convenience state only.
 */
(() => {
  'use strict';
  ...
  function moduleForPath() {
    ...
    if (/\\/laboratory\\//.test(PATH)) return 'laboratory';
    if (/\\/media\\//.test(PATH)) return 'media';
    if (/\\/commerce\\//.test(PATH)) return 'marketplace';
    if (/\\/content\\//.test(PATH)) return 'explore';
  ...
  function render() {
    ...
    nav.innerHTML = `
      ...
      <div class="gei-nav-inner">
        <a class="gei-nav-brand" href="../">GEI<small>PLATFORM</small></a>
        ${window.GEI_PLATFORM_REGISTRY.list().map(m => `<a href="${m.path}" data-module="${m.id}" ${m.id===current?'aria-current="page"':''}>${m.label}</a>`).join('')}
        <span class="gei-nav-status">CORE 1.0.1</span>
      </div>`;
